...
using common;
using static _24.Program;
using Vector3Dec = (decimal X, decimal Y, decimal Z);
namespace _24
{
    public static class GeoExtensions
    {
        public static Vector3Long ZeroZ(this Vector3Long v) => new(v.X, v.Y, 0);

        public static decimal Speed(this Vector3Long step)
        {
            var (x, y, z) = step;
            return ((decimal)Math.Sqrt((double)(x * x + y * y + z * z))).Round();
        }
        public static decimal Round(this decimal d, int places = 5) => Math.Round(d, places);

        public static decimal Time(this Vector3Long v, Vector3Long step)
        {
            return ((decimal)v.X, (decimal)v.Y, (decimal)v.Z).Time(step);
        }

        public static decimal Time(this Vector3Dec v, Vector3Long step)
        {
            var tx = step.X != 0 ? (v.X / (decimal)step.X).Round() : decimal.MaxValue;
            var ty = step.Y != 0 ? (v.Y / (decimal)step.Y).Round() : decimal.MaxValue;
            var tz = step.Z != 0 ? (v.Z / (decimal)step.Z).Round() : decimal.MaxValue;

            if (tx != decimal.MaxValue)
            {
                if (ty != decimal.MaxValue)
                {
                    if (tz != decimal.MaxValue)
                    {
                        if (tx == ty && ty == tz)
                            return tx;
                    ...
                    return tx == ty ? tx : decimal.MaxValue;
                }
            }
            else
            {
...
20, 19, 15 @  1, -5, -3";
        ...
        static long limitLow = 200000000000000;
        static long limitHi = 400000000000000;
        // static long limitLow = 7;
        // static long limitHi = 27;

        static async Task Main(string[] args)
        {
            var strings = input.Split("\r\n", Tidy);
            var hails = strings.Select(h => Hail.Parse(h)).ToList();

            //      Part1(hails);

            Debug.WriteLine("Part2: " + Part2(hails));
        }
        private static void Part1(List<Hail> hails)
        {
            var testarea = (new Vector3Long(limitLow, limitLow, 0), new Vector3Long(limitHi, limitHi, 0));


            var sumOfCrossings = 0;
            foreach (var hail in hails)
            {
                foreach (var hail2 in hails.SkipWhile(x => x != hail).Skip(1))
                {
                    var p = IntersectionPointRayRay3D(hail.Pos.ZeroZ(), hail.Dir.ZeroZ(), hail2.Pos.ZeroZ(), hail2.Dir.ZeroZ());
                    Debug.WriteLine("");
                    Debug.WriteLine($"A: {hail}");
                    Debug.WriteLine($"B: {hail2}");
                    if (p != null)
                    {
                        var pointInRect = PointInRect(p.Value, testarea);

                        var t1 = (p.Value - hail.Pos.ZeroZ()).Time(hail.Dir.ZeroZ());
                        var t2 = (p.Value - hail2.Pos.ZeroZ()).Time(hail2.Dir.ZeroZ());

                        if (t1 < 0 || t2 < 0)
                        {
                            if (t1 < 0 && t2 < 0)
                                Debug.Print("Both crossed earlier");
                            else if (t1 < 0)
                                Debug.Print("B crossed earlier");
                            else if (t2 < 0)
                                Debug.Print("A crossed earlier");
                        }
                        else
                        {
                            if (pointInRect)
                            {
                                sumOfCrossings++;
                                Debug.Print($"Crossing inside at {p}");
                            }
                            else
                            {
                                Debug.Print($"Crossing outside at {p}");
                            }
                        }
                    }
                    else
                    {
                        Debug.Print($"No crossing");
                    }
                }
            }


            Console.WriteLine($"Count of inside crossings: {sumOfCrossings}");
        }
        ...
        private static long Part2(List<Hail> hails)
        {
                ...
                foreach (var x in Range(range))
                {
                    foreach (var y in Range(range))
                    {
                        ...
                        foreach (var z in Range(range))
                        {
                            ...
                            var intersectZ = hails[1].Pos.Z + intersect1.time * (hails[1].Dir.Z + z);
                            var intersectZ2 = hails[2].Pos.Z + intersect2.time * (hails[2].Dir.Z + z);
                            var intersectZ3 = hails[3].Pos.Z + intersect3.time * (hails[3].Dir.Z + z);
                            ...
                            return intersect1.pos.X + intersect1.pos.Y + intersectZ
                                   + Hail.MapPos.X + Hail.MapPos.Y + Hail.MapPos.Z;
                        }
                    }
                }
            }
            ...
            (bool intersects, (long X, long Y) pos, long time)
                TryIntersectPos(Hail one, Hail two, (int x, int y) offset)
            {
                var a = (Pos: (one.Pos.X, one.Pos.Y), Vel: (X: one.Dir.X + offset.x, Y: one.Dir.Y + offset.y));
                var c = (Pos: (two.Pos.X, two.Pos.Y), Vel: (X: two.Dir.X + offset.x, Y: two.Dir.Y + offset.y));

                //Determinant
                long D = (a.Vel.X * -1 * c.Vel.Y) - (a.Vel.Y * -1 * c.Vel.X);
        ...
        public static bool PointInRect(Vector3Dec p1, (Vector3Long p1, Vector3Long p2) rect) =>
            (p1.X >= rect.p1.X && p1.X <= rect.p2.X &&
             p1.Y >= rect.p1.Y && p1.Y <= rect.p2.Y);

        private static (decimal X, decimal Y, decimal Z)? IntersectionPointRayRay3D(
            Vector3Long line1Point1, Vector3Long line1Direction,
            Vector3Long line2Point1, Vector3Long line2Direction)
        {
            Vector3Long line1ToLine2 = line1Point1 - line2Point1;

            var a = Vector3Long.DotProduct(line1Direction, line1Direction);
            var b = Vector3Long.DotProduct(line1Direction, line2Direction);
            var c = Vector3Long.DotProduct(line2Direction, line2Direction);
            var d = Vector3Long.DotProduct(line1Direction, line1ToLine2);
            var e = Vector3Long.DotProduct(line2Direction, line1ToLine2);
            ...
            var (x, y, z) = line1Direction;
            var vector3Dec = (X: x * t, Y: y * t, Z: z * t);
            vector3Dec.X += line1Point1.X;
            vector3Dec.Y += line1Point1.Y;
            vector3Dec.Z += line1Point1.Z;
            return vector3Dec;
        }

        internal class Hail
        {
            public static Vector3Long MapPos { get; set; } = new Vector3Long(long.MaxValue, long.MaxValue, long.MaxValue);

            public static void MoveAll(List<Hail> hails)
            {
                hails.ForEach(h =>
                {
                    MapPos = MapPos with { X = Math.Min(MapPos.X, h.Pos.X) };
                    MapPos = MapPos with { Y = Math.Min(MapPos.Y, h.Pos.Y) };
                    MapPos = MapPos with { Z = Math.Min(MapPos.Z, h.Pos.Z) };
                });
                MapPos += (-1);
                hails.ForEach(h => h.Pos -= MapPos);
            }
            public static void MoveBackAll(List<Hail> hails)
            {
                hails.ForEach(h => h.Pos += MapPos);
            }

            private Hail(Vector3Long pos, Vector3Long dir)
            {
                Id = ++counter;
                Pos = pos;
                Dir = dir;
            ...
            //499.138.453.043.411
            //400.000.000.000.000
            public Vector3Long Pos { get; set; }
            public Vector3Long Dir { get; }
            public decimal Tod { get; set; } = decimal.MaxValue;

            private static int counter = 0;
            public static Hail Parse(string s)
            {
                var p = s
                    .Split(" ,@".ToCharArray(), Tidy)
                    .Select(x => x.ToLong() ?? 0)
                    .ToList();
                return new Hail(new Vector3Long(p[0], p[1], p[2]), new Vector3Long(p[3], p[4], p[5]));

            }

            public Vector3Long PosAtStep(long step) => Pos + (Dir * step);

            private Hail VOffsetXy(Vector3Long offs)
            {
                return new Hail(Pos.ZeroZ(), new Vector3Long(Dir.X + offs.X, Dir.Y + offs.Y, 0));
            }
            public decimal InterpolateZ(decimal t, long dvz) => (Pos.Z + t * (Dir.Z + dvz)).Round();

            private (bool, Vector3Long, decimal) IntersectsXy(Hail other)
            {
                if (Dir.X == 0) return (false, new Vector3Long(0, 0, 0), -1);
                var dydx = Dir.Y / Dir.X;
                var c = Pos.Y - dydx * Pos.X;

                if (other.Dir.X == 0) return (false, new Vector3Long(0, 0, 0), -1);
                var hdydx = other.Dir.Y / other.Dir.X;
                var hc = other.Pos.Y - hdydx * other.Pos.X;

                if (dydx == hdydx) return (false, new Vector3Long(0, 0, 0), -1);

                var x = (hc - c) / (dydx - hdydx);
                var t1 = (x - Pos.X) / Dir.X;
                var t2 = (x - other.Pos.X) / other.Dir.X;

                if (t1 < 0 || t2 < 0) return (false, new Vector3Long(0, 0, 0), -1);

                var y = dydx * (x - Pos.X) + Pos.Y;

                return (true, new Vector3Long(x, y, 0), t1);
            }

            public (bool intersects, Vector3Long p, decimal t)
                IsIntersectionXy(Hail other, Vector3Long d)
                => other.VOffsetXy(d).IntersectsXy(VOffsetXy(d));

            public override string ToString()
            {
                return $"{(Id > 26 ? "H" + Id : "" + (char)('A' - 1 + Id))}: {Pos} {Dir}";
            }
        }

        public record struct Vector3Long(long X, long Y, long Z)
        {
            public static explicit operator Vector3D(Vector3Long d) => new Vector3D(d.X, d.Y, d.Z);
            public static implicit operator Vector3Dec(Vector3Long d) => (d.X, d.Y, d.Z);
            public static long DotProduct(Vector3Long vector1, Vector3Long vector2)
            {
                return DotProduct(ref vector1, ref vector2);
            }

            internal static long DotProduct(ref Vector3Long vector1, ref Vector3Long vector2)
            {
                return vector1.X * vector2.X + vector1.Y * vector2.Y + vector1.Z * vector2.Z;
            }
            public static Vector3Long operator *(Vector3Long vector, long scalar)
            {
                return new Vector3Long(vector.X * scalar, vector.Y * scalar, vector.Z * scalar);
            }
            public static Vector3D operator *(Vector3Long vector, double scalar)
            {
                return new Vector3D(vector.X * scalar, vector.Y * scalar, vector.Z * scalar);
            }
            public static Vector3Long operator /(Vector3Long vector, long scalar)
            {
                var tx = scalar != 0 ? (vector.X / scalar) : long.MaxValue;
                var ty = scalar != 0 ? (vector.Y / scalar) : long.MaxValue;
                var tz = scalar != 0 ? (vector.Z / scalar) : long.MaxValue;

                return new(tx, ty, tz);
            }

            public static Vector3Long operator /(Vector3Long v, Vector3Long step)
            {
                var tx = step.X != 0 ? (v.X / step.X) : long.MaxValue;
                var ty = step.Y != 0 ? (v.Y / step.Y) : long.MaxValue;
                var tz = step.Z != 0 ? (v.Z / step.Z) : long.MaxValue;

                return new(tx, ty, tz);
            }

            public static Vector3Long operator -(Vector3Long p1, Vector3Long p2)
            {
                return new Vector3Long(p1.X - p2.X, p1.Y - p2.Y, p1.Z - p2.Z);
            }
            public static Vector3Dec operator -(Vector3Dec p1, Vector3Long p2)
            {
            ...
            public static Vector3Long operator +(Vector3Long p1, long scalar)
            {
                return new Vector3Long(p1.X + scalar, p1.Y + scalar, p1.Z + scalar);
            }
        };
