...
fivebxsevensixone872dlx";
            var replacements = "one:1, two:2, three:3, four:4, five:5, six:6, seven:7, eight:8, nine:9";
            var replacers = replacements.Split(',', StringSplitOptions.TrimEntries)
                .Select(x => x.Split(':'));


            var inputLines = input.Split('\n');
            var sum = inputLines.Select(x => GetNumbersInLine(x, replacers)).Sum();
            Debug.WriteLine(sum);
        }

        private static long GetNumbersInLine(string line, IEnumerable<string[]> replacers)
        {
            List<(int pos, string value)> hits = new List<(int pos, string value)>();
            foreach (var replacer in replacers)
            {
                var pos = line.IndexOf(replacer[0], 0);
                while (pos >= 0)
                {
                    hits.Add((pos, replacer[1]));
                    pos = line.IndexOf(replacer[0], pos + replacer[0].Length);
                }
                pos = line.IndexOf(replacer[1], 0);
                while (pos >= 0)
                {
                    hits.Add((pos, replacer[1]));
                    pos = line.IndexOf(replacer[1], pos + 1);
                }
            }

            hits = hits.OrderBy(x => x.pos).ToList();
            return ((hits.First().value + hits.Last().value))?.ToLong() ?? 0;
        }
    }
}