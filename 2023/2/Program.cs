...
namespace _2
{
    internal class Program
    {
        private class Game
        {
            ...
            public List<Dictionary<string, long>> Picks { get; set; }

            public bool IsPossible(Dictionary<string, int> bagLimits)
            {
                return Picks
                    .All(pick => !pick.Keys
                        .Any(color => bagLimits[color] < pick[color]));
            }
            public long CalculateProduct
            {
                get =>
                    Picks.SelectMany(x => x.Keys
                            .Select(y => (color: y, cnt: x[y])))
                        .ToList()
                        .GroupBy(x => x.color)
                        .Select(x => (color: x.Key, cnt: x.Max(x => x.cnt)))
                        .Aggregate(1L, (current, tuple) => current * tuple.cnt);
            }
        }
        ...
        static void Main(string[] args)
        {
            var games = input.Split("\r\n", Tidy)
                .Select(x =>
                {
                    var game = x.Split(":", Tidy);
                    var picks = game[1].Split(";", Tidy)
                        .Select(aDraw =>
                            aDraw.Split(",", Tidy)
                                .Select(drawn =>
                                {
                                    var a = drawn.Split(" ", Tidy);
                                    return (color: a[1], cnt: a[0].ToLong() ?? 0);
                                })
                                .ToDictionary(z => z.color, z => z.cnt));

                    return new Game()
                    {
                        Id = game[0].Split(" ")[1].Trim().ToLong() ?? 0,
                        Picks = picks.ToList()
                    };
                });

            var bagLimits = new Dictionary<string, int>()
            {
                {"red",12},
                {"green",13},
                {"blue",14}
            ...
            var possibleGames = games.Where(x => x.IsPossible(bagLimits)).ToList();
            Debug.WriteLine(possibleGames.Sum(x => x.Id));

            Debug.WriteLine(games.Sum(x => x.CalculateProduct));
