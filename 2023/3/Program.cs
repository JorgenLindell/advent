...
namespace _3
{
    internal class Program
    {
        ...
        {
            List<string> matrix = input.Split("\r\n", Tidy).Select(x => ".........." + x + "..........").ToList();
            var linelength = matrix[0].Length;
            matrix.Insert(0, "".PadLeft(linelength, '.'));
            matrix.Add("".PadLeft(linelength, '.'));
            var reNumber = new Regex("[0-9]+");
            var numbers = matrix.SelectMany((x, i) => reNumber.Matches(x).Select(y =>
            {
                return new Hit()
                {
                    Line = i,
                    Number = y.Value.ToLong(),
                    Start = y.Index,
                    Length = y.Length
                };
            })).ToList();

            Func<char, bool> testSymbol = (c) => !"1234567890.".Contains(c);

            var numbersWithSymbol = numbers.Where(x => x.HasSymbol(matrix, testSymbol).Any()).ToList();
            Debug.WriteLine("first:"+numbersWithSymbol.Sum(x => x.Number));

            var symbolsStar = new Dictionary<(int, int), List<Hit>>();
            var numbersWithStar = numbers.Where(x =>
            {
                var symbols = x.HasSymbol(matrix, c => c == '*');
                if (!symbols.Any())
                    return false;

                foreach (Symbol symbol in symbols)
                {
                    if (symbolsStar.ContainsKey(symbol.Key))
                        symbolsStar[symbol.Key].Add(symbol.Hits.First());
                    else
                        symbolsStar[symbol.Key] = new List<Hit>() { symbol.Hits.First() };
                }
                return true;
            }).ToList();

            var total2=symbolsStar.Values.Where(x => x.Count == 2).Select(x => x[0].Number * x[1].Number).Sum();
        ...
        private class Hit
        {
            ...
            public List<Symbol> HasSymbol(List<string> matrix, Func<char, bool> v)
            {
                int lineIx;
                int colIx = 0;
                var result = new List<Symbol>();
                for (lineIx = Line - 1; lineIx < Line + 2; ++lineIx)
                {
                    for (colIx = Start - 1; colIx < Start + Length + 1; ++colIx)
                    {
                        if (v(matrix[lineIx][colIx]))
                        {
                            result.Add(new Symbol(lineIx, colIx, this));
                        }
                    }
                }
                return result; ;
        ...
        private class Symbol
        {
            ...
            public (int Line, int Col) Key => (lineIx, colIx);
            public List<Hit> Hits { get; private set; }

            public Symbol(int lineIx, int colIx, Hit hit)
            {
                this.lineIx = lineIx;
                this.colIx = colIx;
                this.Hits = new List<Hit>() { hit };
            }
        }
